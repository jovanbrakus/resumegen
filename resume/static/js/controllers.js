...
function ResumeCtrl($scope) {
    ...
    PopulateSWEngineeringTemplate($scope.resume);

    $scope.addSegment  = function () {
        $scope.resume.addSegment() ;       
    };
    ...
    $scope.generateResume = function () {
        $.ajax({
                ...
                data: {'resume': JSON.stringify($scope.resume)},
                dataType: 'text',
                success: function(result) {window.open('data:application/pdf;base64,' + Base64.encode(result));}
        });
    };

    $scope.populateSampleResume = function () {
        PopulateSampleResume($scope.resume);
    };
